        ...
        if request.headers.get('x-requested-with') == 'XMLHttpRequest': 
            try:
                ...
                try:
                    ...
                    return JsonResponse({'status': 'Success', 'message':'Added', 'cart_counter': get_cart_counter(request),"qty": chkCart.quantity, 'cart_amount':get_cart_amounts(request)})
            except:
    ...
    if request.user.is_authenticated:
        if request.headers.get('x-requested-with') == 'XMLHttpRequest': 
            try:
                ...
                return JsonResponse({'status': 'Failed', 'message':'food does not exists'})
        else:
            return JsonResponse({'status': 'Failed', 'message':'Invalid request'})
    else:
        return JsonResponse({'status': 'login_required', 'message': 'Please login to continue'})
